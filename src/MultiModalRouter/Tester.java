...
public class Tester {
    private static final long NUMBER_MULTI_MODAL_QUERIES = 1_000_000;
    private static final double AVERAGE_WALKING_SPEED_M_PER_MIN = 85.2;     // Translates to 1.4 m/s
    private static final double MAXIMUM_WALKING_DISTANCE_M = 1_500;     // todo check redundance
    ...
    public static void main(String[] args) {
        ...
        // Execute one query on JMMR structure
        // Parse query data
        double originPointLongitude = multiModalQueries.get(1).getOriginLongitude();
        double originPointLatitude = multiModalQueries.get(1).getOriginLatitude();
        double destinationPointLongitude = multiModalQueries.get(1).getDestinationLongitude();
        double destinationPointLatitude = multiModalQueries.get(1).getDestinationLatitude();
        int originPointDepartureTime = multiModalQueries.get(1).getDepartureTime();

        // Find the nearest nodes for origin and destination points
        ...
        // Collect nearest stops for nodes closest to origin and destination points
    ...
    private static ArrayList<Double> findTravelTimesFromOriginToOriginStops(long originNodeId,
        ...
        for (Stop stopNearOriginNode : stopsNearOrigin) {
            ...
            double dijkstraTravelTime = dijkstraBasedRouter.findShortestDrivingPathCostMin(originNodeId,   // todo this might be super inefficient, instantiate one dijkstra and throw it everywhere
    ...
    private static ArrayList<Double> findTravelTimesFromDestinationStopsToDestination
        ...
        for (Stop stopNearDestinationNode : stopsNearDestination) {
            ...
            DijkstraBasedRouter dijkstraBasedRouter = new DijkstraBasedRouter();
            double dijkstraTravelTime = dijkstraBasedRouter.findShortestDrivingPathCostMin(destinationNodeId,
